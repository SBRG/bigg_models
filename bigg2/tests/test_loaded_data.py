...
def _check_merged(cobra_dictlist, model_bigg_id, cobra_type):
    ...
    """
    # find merged genes
    db_merged = [(g.id, g.notes['original_bigg_ids']) for g in cobra_dictlist]
    ...
    if db_merged_extra > 0:
        merged_dict = {x[0]: x[1] for x in db_merged if len(x[1]) > 1}
...
def test_metabolite_count(db_model, pub_model):
    ...
    assert db_metabolites_len + db_merged_extra == pub_metabolites_len
