...
for model_file in dirs:
    ...
    try:
        ...
        bigg_id = model.id
        format = 'xml'
        #target_dir = join(root_directory, 'static', 'published_models',
        #'%s_published.%s' % (bigg_id, format)
        ...
        if not os.path.exists(target_dir):
            ...
            shutil.copy(join(model_dir, model_file),
                        join(target_dir,'%s_published.%s' % (bigg_id, format)))
        else:
            shutil.copy(join(model_dir, model_file),
                        join(target_dir,'%s_published.%s' % (bigg_id, format)))
        #os.rename(join(target_dir, model_file), '%s_published.%s' % (bigg_id,
        #format) )
    except:
